	...
	switch (command.slice(1).toLowerCase()) {
		...
		case "add":
			if (youtubeSearched === true) {
				...
				message.reply("you need to use 1yt [search term] command first to add song from the list to the queue.");
			}
			break;
...
.setThumbnail(message.author.avatarURL)
	.setDescription (`
#User "3userinfo [username or blank]": displays info for the selected user or for message sender if arguments are not provided

#Music "3play [song name or blank]": plays a song or if arguments are not provided plays the first song in the queue

"3skip": skips the current song

"3queue": displays current queue

"3queue remove [song number]": removes chosen song from the queue

"3queue clear": removes all songs from the queue

"3queue shuffle": shuffles the current queue

"3repeat": plays current song again

"3stop": stops playing music and deletes all songs in the queue

"3yt [search term]": searches the YouTube and returns first 5 results

"3add [search result number]": adds a song from YouTube search to the queue

"3vol [percentage]": sets the volume of the music to given percentage
